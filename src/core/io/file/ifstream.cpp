...
File* FileInputStream::open(const char* path, FileMode mode) {
...
b32 FileInputStream::is_open() const {
    return file_is_open(m_file);
}

u64 FileInputStream::size() const {
    return file_size(m_file);
}

u64 FileInputStream::position() const {
