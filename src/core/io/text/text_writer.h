...
private:
    ...
    template<typename T>
    static String format_value(T val) {
...
template<>
inline TextWriter& TextWriter::operator<<(const char* const& val) {
    m_writer.write(val, std::char_traits<char>::length(val));
    return *this;
}

template<>
inline TextWriter& TextWriter::operator<<(const String& val) {
    m_writer.write(val.data(), val.size());
    return *this;
}

template<>
inline TextWriter& TextWriter::operator<<(const char& val) {
