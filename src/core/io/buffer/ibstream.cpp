...
BufferInputStream::BufferInputStream(IStream *input, u64 buffer_size)
    : m_input(input), m_buffer(buffer_size > 0 ? buffer_size : DEFAULT_SIZE),
      m_buffer_size(buffer_size > 0 ? buffer_size : DEFAULT_SIZE) {}

BufferInputStream::~BufferInputStream() { close(); }
...
b32 BufferInputStream::is_open() const { return m_input && m_input->is_open(); }

u64 BufferInputStream::size() const { return m_input ? m_input->size() : 0; }
