...
template <typename T> void Array<T>::reserve(u64 newCapacity) {
...
template <typename T> u64 Array<T>::size() const { return m_size; }
...
template <typename T> const T *Array<T>::data() const { return m_data; }

template <typename T> void Array<T>::push_back(const T &value) {
  if (m_size == m_capacity) {
    u64 newCap =
        m_capacity == 0 ? 2 : m_capacity + (m_capacity >> 1); // 1.5x, integer
    reserve(newCap);
  }
...
template <typename T> void Array<T>::insert(u64 index, T &&value) {
  SV_ASSERT(index <= m_size, "Index {} out of bound", index);
...
template <typename T> void Array<T>::append(const T *data, u64 size) {
  u64 oldSize = m_size;
  resize(m_size + size);
...
template <typename T> void Array<T>::erase(u64 index) {
  if (m_size == 0) {
    return;
  }
