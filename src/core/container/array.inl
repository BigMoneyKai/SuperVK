...
: m_allocator(a), m_size(size) {
    m_capacity = m_size * GROWTH_FACTOR;
    if(a) {
        m_data = static_cast<T*>(a->allocate(m_capacity * sizeof(T)));
    } else {
        m_data = static_cast<T*>(platform_aligned_alloc(m_capacity * sizeof(T), ALIGNMENT));
    }
...
void Array<T>::reserve(u64 newCapacity) {
    ...
    if(m_allocator) {
        ...
        newData = static_cast<T*>(platform_aligned_alloc(newCapacity * sizeof(T), ALIGNMENT));
    }
...
void Array<T>::resize(u64 newSize, const T& value) {
    ...
    if(m_allocator) {
        ...
        newData = static_cast<T*>(platform_aligned_alloc(newSize * sizeof(T), ALIGNMENT));
    }
    ...
    m_capacity = GROWTH_FACTOR * newSize;
    m_size = newSize;
    m_data = newData;
}
...
template <typename T>
u64 Array<T>::size() const {
    return m_size;
}

template <typename T>
u64 Array<T>::capacity() const {
    return m_capacity;
}

template <typename T>
b32 Array<T>::empty() const {
    return m_size == 0;
}
...
void Array<T>::insert(u64 index, const T& value) {
    if(index > m_size) {
        FATAL("Index %lu out of bound", index);
        return;
    }
