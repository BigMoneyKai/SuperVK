...
template <typename T> Queue<T>::Queue(Allocator *a) : m_allocator(a) {}

template <typename T>
Queue<T>::Queue(u64 size, const T &value, Allocator *a)
...
template <typename T> void Queue<T>::reserve(u64 newCapacity) {
...
template <typename T> void Queue<T>::resize(u64 newSize, const T &value) {
  ...
  m_capacity = newSize + (newSize >> 1);
  m_size = newSize;
  m_data = newData;
}

template <typename T> void Queue<T>::shrink_to_fit() {
  if (m_size == m_capacity || m_size == 0 || !m_data) {
    return;
  }

  T *newData = nullptr;
  if (m_allocator) {
    newData = static_cast<T *>(m_allocator->allocate(m_size * sizeof(T)));
  } else {
    newData =
        static_cast<T *>(platform_aligned_alloc(m_size * sizeof(T), ALIGNMENT));
  }
...
template <typename T> Allocator *Queue<T>::allocator() const {
  return m_allocator;
}

template <typename T> u64 Queue<T>::size() const { return m_size; }

template <typename T> u64 Queue<T>::capacity() const { return m_capacity; }

template <typename T> b32 Queue<T>::empty() const { return m_size == 0; }

template <typename T> T &Queue<T>::operator[](u64 i) {
  SV_ASSERT(i < m_size, "Queue index {} out of bound", i);
  return m_data[i];
}

template <typename T> const T &Queue<T>::operator[](u64 i) const {
  SV_ASSERT(i < m_size, "Queue index {} out of bound", i);
  return m_data[i];
}
...
template <typename T> T *Queue<T>::data() { return m_data; }

template <typename T> const T *Queue<T>::data() const { return m_data; }

template <typename T> void Queue<T>::push_back(const T &value) {
...
template <typename T> void Queue<T>::pop_front() {
  ...
  u64 tail = m_size - 1;
  if (tail) {
    // For trivially copyable types, use memmove
    if constexpr (std::is_trivially_copyable_v<T>) {
      memmove(m_data, m_data + 1, tail * sizeof(T));
    } else {
...
T &Queue<T>::emplace_back(Args &&...args) {
  if (m_size == m_capacity) {
    u64 newCap = m_capacity == 0 ? 2 : m_capacity + (m_capacity >> 1);
    reserve(newCap);
  }
...
template <typename T> void Queue<T>::insert(u64 index, const T &value) {
  ...
  if constexpr (std::is_trivially_copyable_v<T>) {
    ...
    if (tail)
      memmove(m_data + index + 1, m_data + index, tail * sizeof(T));
  } else {
...
template <typename T> void Queue<T>::insert(u64 index, T &&value) {
  SV_ASSERT(index <= m_size, "Queue index {} out of bound", index);
...
template <typename T> void Queue<T>::erase(u64 index) {
  if (m_size == 0) {
    return;
  }
  SV_ASSERT(index < m_size, "Queue index {} out of bound", index);
  ...
  if constexpr (std::is_trivially_copyable_v<T>) {
    ...
    if (tail)
      memmove(m_data + index, m_data + index + 1, tail * sizeof(T));
  } else {
    m_data[index].~T();
