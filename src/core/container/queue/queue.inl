...
template <typename T>
void Queue<T>::resize(u64 newSize, const T& value) {
...
void Queue<T>::shrink_to_fit() {
    ...
    if (m_allocator) {
        ...
        newData = static_cast<T*>(
            platform_aligned_alloc(m_size * sizeof(T), ALIGNMENT));
    }
...
template <typename T>
Allocator* Queue<T>::allocator() const {
    return m_allocator;
}

template <typename T>
u64 Queue<T>::size() const { return m_size; }

template <typename T>
u64 Queue<T>::capacity() const { return m_capacity; }

template <typename T>
b32 Queue<T>::empty() const { return m_size == 0; }
...
template <typename T>
T* Queue<T>::data() { return m_data; }

template <typename T>
const T* Queue<T>::data() const { return m_data; }
