...
template <typename K, typename V, typename Entry>
HashMap<K, V, Entry> &
HashMap<K, V, Entry>::operator=(HashMap<K, V, Entry> &&other) noexcept {
  if (this != &other) {
...
template <typename K, typename V, typename Entry>
void HashMap<K, V, Entry>::insert(const K &key, const V &val) {
  insert_impl(key, val);
}
template <typename K, typename V, typename Entry>
void HashMap<K, V, Entry>::insert(const K &key, V &&val) {
  insert_impl(key, std::move(val));
}
template <typename K, typename V, typename Entry>
void HashMap<K, V, Entry>::insert(K &&key, const V &val) {
  insert_impl(std::move(key), val);
}
template <typename K, typename V, typename Entry>
void HashMap<K, V, Entry>::insert(K &&key, V &&val) {
  insert_impl(std::move(key), std::move(val));
}
template <typename K, typename V, typename Entry>
void HashMap<K, V, Entry>::insert(const Entry &entry) {
  insert_impl(entry.key, entry.val);
}
template <typename K, typename V, typename Entry>
void HashMap<K, V, Entry>::insert(Entry &&entry) {
  insert_impl(std::move(entry.key), std::move(entry.val));
}
...
void HashMap<K, V, Entry>::rehash(u64 newCapacity) {
  ...
  m_mask = m_capacity - 1;
  m_keys = Array<K>(m_allocator);
  m_keys.resize(m_capacity);
  m_vals = Array<V>(m_allocator);
  m_vals.resize(m_capacity);
  m_hashKeys = Array<u64>(m_allocator);
  m_hashKeys.resize(m_capacity);
...
void HashMap<K, V, Entry>::insert_impl(Key &&key, Val &&val) {
  if ((m_size + 1) >= static_cast<u64>(static_cast<f64>(m_capacity) *
                                       static_cast<f64>(REHASH_FACTOR))) {
    rehash(m_capacity * 2);
  }
