...
public:
    ...
    using const_iterator = HashMapIterator<Entry>;
	...
	b32 contains(const K& key);

	V& operator[](const K& key);
	const V& operator[](const K& key) const;

	SV_FORCE_INLINE iterator begin() {
		auto* k = m_keys.data();   auto* v = m_vals.data();
		auto* h = m_hashKeys.data(); auto* d = m_probeDists.data();
		auto* o = m_occupied.data();
		auto ei = m_capacity; u64 i = 0;
		while (i != ei && !o[i]) ++i;
...
private:
	...
	void insert_impl(Key&& key, Val&& val);
