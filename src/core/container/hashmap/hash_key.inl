...
SV_FORCE_INLINE void hash_key_combine(u64 &seed, const T &value) {
  seed ^=
      hash_key<T>{}(value) + 0x9e3779b97f4a7c15ULL + (seed << 6) + (seed >> 2);
}
...
template <> struct hash_key<u8> {
  u64 operator()(u8 key) const {
    return hash_key<u64>{}(static_cast<u64>(key));
  }
};
...
template <> struct hash_key<i8> {
  u64 operator()(i8 key) const {
    return hash_key<u64>{}(static_cast<u64>(key));
  }
};

template <> struct hash_key<i16> {
  u64 operator()(i16 key) const {
    return hash_key<u64>{}(static_cast<u64>(key));
  }
};
...
template <> struct hash_key<f32> {
  u64 operator()(f32 key) const {
    return hash_key<u64>{}(std::bit_cast<u32>(key));
  }
};

template <> struct hash_key<f64> {
  u64 operator()(f64 key) const {
    return hash_key<u64>{}(std::bit_cast<u64>(key));
  }
};
...
template <typename T> u64 hash_key<T *>::operator()(const T *key) const {
