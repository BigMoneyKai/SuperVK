...
template <typename T>
static SV_FORCE_INLINE T max(T a, T b) {
    return a > b ? a : b;
}

static SV_FORCE_INLINE u64 min(u64 a, u64 b) {
    return a < b ? a : b;
}
...
inline void String::grow(u64 minCapacity) {
    ...
    if (!m_heap) {
        ...
        if (m_allocator)
            ...
            newData = static_cast<char*>(platform_aligned_alloc(newCapacity + 1, 16));
...
SV_FORCE_INLINE const char* String::c_str() const {
    return data_ptr();
}
...
SV_FORCE_INLINE u64 String::size() const {
    return m_size;
}

SV_FORCE_INLINE u64 String::length() const {
    return m_size;
}
...
SV_FORCE_INLINE b32 String::empty() const {
    return m_size == 0;
}
...
SV_FORCE_INLINE void String::resize(u64 count) {
    resize(count, '\0');
}
...
SV_FORCE_INLINE Allocator* String::allocator() const {
    return m_allocator;
}
