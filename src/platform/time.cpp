...
void timer_init(Timer *t) { t->start_ns = monotonic_ns(); }

void timer_start(Timer *t) { t->start_ns = monotonic_ns(); }

void timer_reset(Timer *t) { t->start_ns = monotonic_ns(); }
...
f64 timer_now_ms(void) { return (f64)monotonic_ns() / 1000000.0; }
...
static u64 monotonic_ns(void) { return (u64)counter_to_ns(query_counter()); }
...
void timer_sleep_ms(u64 ms) { Sleep((DWORD)ms); }
