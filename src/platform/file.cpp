...
i32 file_is_open(const FileHandle *f) { return f && f->fp ? 1 : 0; }
...
i32 fs_file_delete(const char *path) { return remove(path) == 0 ? 0 : -1; }
...
FileHandle *file_open(const char *path, FileMode mode) {
  FileHandle *f = new FileHandle{};
  f->h = CreateFileA(path, win_access(mode), FILE_SHARE_READ, nullptr,
                     win_creation(mode), FILE_ATTRIBUTE_NORMAL, nullptr);
...
i32 file_is_open(const FileHandle *f) { return f && f->h ? 1 : 0; }

i32 file_is_eof(const FileHandle *f) {
  if (!f)
    return 1;
  LARGE_INTEGER li = {}, pos = {}, sz = {};
  if (!SetFilePointerEx(f->h, li, &pos, FILE_CURRENT))
    return 1;
...
i32 fs_file_delete(const char *path) { return DeleteFileA(path) ? 0 : -1; }
