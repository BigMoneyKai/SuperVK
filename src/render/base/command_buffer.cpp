...
void CommandBuffer::init(const VkDevice &device, VkCommandPool pool,
                         u32 count) {
  m_device = device;
  m_pool = pool;
  m_commandBuffers.resize(count);

  m_commandBufferAllocateInfo.sType =
  ...
  VK_CHECK_RESULT(vkAllocateCommandBuffers(
      m_device, &m_commandBufferAllocateInfo, m_commandBuffers.data()));
}
