...
void Shader::init(const VkDevice& device, const char* path, ShaderType type) {
    ...
    m_shaderModuleCreateInfo.codeSize = static_cast<size_t>(m_code.size() * sizeof(u32));
    m_shaderModuleCreateInfo.pCode = m_code.data();

    VK_CHECK_RESULT(vkCreateShaderModule(device, &m_shaderModuleCreateInfo, nullptr, &m_shaderModule));
