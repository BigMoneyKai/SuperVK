...
void Pipeline::init(const VkDevice &device, const VkRenderPass &renderPass,
  ...
  pipelineLayoutInfo.setLayoutCount =
      static_cast<u32>(desc.descriptorSetLayouts.size());
  pipelineLayoutInfo.pSetLayouts = desc.descriptorSetLayouts.empty()
                                       ? nullptr
  ...
  depth.depthCompareOp = desc.compareOp;
  ...
  attachment.blendEnable = desc.blendingEnabled;
  ...
  VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &info,
                                            nullptr, &m_pipeline));
}
