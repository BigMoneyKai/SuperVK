...
void Texture::init(const VkDevice &device,
                   ...
                   const VkQueue &graphicsQueue, const char *path) {
  m_device = device;
  ...
  m_image.init(m_device, m_physicalDevice, m_commandPool, m_graphicsQueue,
               imageDesc);
...
void Texture::copyBufferToImage(const VkExtent3D &extent) {
  ...
  vkCmdCopyBufferToImage(cmd, m_stagingBuffer.buffer(), m_image.image(),
                         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);

  endSingleTimeCommands(m_device, m_graphicsQueue, m_commandPool, cmd);
}
